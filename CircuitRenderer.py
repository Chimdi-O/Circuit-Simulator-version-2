...
class Component(): 
    def __init__(self,master): 
        ...
        self.geometry = []     
        self.circle_size = 4
        self.collison_box_coords = [[]]
    
    def create_line(self,master,angle,length,line_pos): #the angle is in reference to the line between the start and end point and the line pos is the position along that line
        
        vector = GeometryMath.find_vector(self.start_pos,self.end_pos)
        norm_vector = GeometryMath.normalise_vector(vector)
        norm_rotated_vector = GeometryMath.rotate_vector(norm_vector,angle)
        scaled_norm_vector = GeometryMath.scale_vector(norm_vector,line_pos)
        scaled_norm_rotated_vector = GeometryMath.scale_vector(norm_rotated_vector,length)
        top_of_line , bottom_of_line = GeometryMath.find_line_coords(scaled_norm_vector,scaled_norm_rotated_vector,self.start_pos)

        self.geometry.append(master.canvas.create_line(top_of_line[0],top_of_line[1],bottom_of_line[0],bottom_of_line[1],fill="black",width=3))
...
class Wire(Component): 
    ...
    def find_collision_box_edges(self): 
...
class Resistor(Wire): 
     ...
     def draw_wire(self,master): 
        ...
        self.geometry = [master.canvas.create_line( self.start_pos[0],self.start_pos[1], self.gap_start[0], self.gap_start[1], fill="black", width=3),
               ...
               master.canvas.create_text(self.start_pos[0], self.start_pos[1], text=self.resistance, fill="black", font=('Helvetica 15 bold'))
                                   ] 
     ...
     def move_line(self,master):  
        ...
        master.canvas.coords(self.geometry[4],self.bottom_left[0],self.bottom_left[1],
                                   ...
                                   self.top_right[0],self.top_right[1],
                                   self.top_left[0],self.top_left[1])
        master.canvas.coords(self.geometry[5],self.text_location[0],self.text_location[1])

     def place_text(self):
         
         if self.start_pos == self.end_pos: 
            self.text_location = self.start_pos
            return
        ...
        #calculates the vector starting at start pos 
         ...
         scaled_norm_tx = norm_px*text_scale
         scaled_norm_ty = norm_py*text_scale

         #middle 
         midx = (self.end_pos[0] + self.start_pos[0])/2
         midy = (self.end_pos[1] + self.start_pos[1])/2

         self.text_location  = [midx - scaled_norm_tx, midy - scaled_norm_ty ]
